...
class TestCreateCourier:
    ...
    def test_create_courier_success(self, courier_cleaner):
        ...
        courier_cleaner(payload)

        assert response.status_code == 201
        assert response.json() == {"ok": True}

    @allure.title("Duplicate courier cannot be created")
    def test_create_duplicate_courier_returns_error(self, courier_cleaner):
        payload = generate_courier_payload()
    ...
    def test_create_courier_without_first_name_returns_error(self, courier_cleaner):
        payload = generate_courier_payload()
